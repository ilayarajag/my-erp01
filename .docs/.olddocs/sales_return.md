...
```json
{
  ...
  "cess_amt": "number",
  "roff": "number",
  "billno": "string", // Select Sales/Transfer
  "company_id": "integer",
...
3. `PartyLedger` table below should affect
   ...
   - `Debit` - `0`
   - `Credit` - column should be post - `sales_return_master_total_amount`
   - `Type` cloumn should be post - `SR - bill of docs number`
   - `Mode` to be empty
...
4. `StockLedger` table below should affect
   ...
   - if row exsists update exsisting `sales_return_qty` with `qty` from the sales details contracts row against `date and prodid`
   ...
   - free_qty - `0`
   - sales_in_qty - `0`
   - sales_return_qty - `exsisting sales_return_qty` + `qty from the sales details contracts`
   - tr_in_qty - `0`
   - tr_out_qty - `0`
...
7. `OutletStockLedger` table below should affect
   ...
   - Note: if row does not exsists against `date and prodid` insert new row
   - For a date and prodid only one row should be there.
   - if row exsists update exsisting `tr_out_qty` with `qty` from the sales details contracts row against `date and prodid` and `outletid`
   ...
   - sales_return_qty - `0`
   - tr_in_qty - `0`
   - tr_out_qty - ` exsisting tr_out_qty` + `qty from the sales details contracts`
   - company_id - `from contracts details`
   - created_by - `users id who are doing the operations`
   - created_at - `date time stamp from backend`
   - updated_by - `users id who are doing the operations`
