# Purchase Return Module Logics

## Tables

    1. PurchaseReturnMaster
        - id
        - docno
        - docdate
        - partycode
...
- by passing docno , need to fetch all purchase_mst and purchase_detl

1. After inserting above into respective tables. need to update `customer/suppliers` table `balance` column
2. Formula:
   `new_balance = exsisting_balance - purchase_return_master_total_amount`
3. `PartyLedger` table below should affect
   - `id` - `auto increment`
   - `Edate` - `date to be posted`
   - `PartyCode` - `this is supplier/party id or code`
   - `Debit` - `0`
   - `Credit` - column should be post - `purchase_return_master_total_amount`
   - `Type` cloumn should be post - `Purchase Return(PR) - bill of docs number`
   - `Mode` to be empty
4. `StockLedger` table below should affect
   ...
   - if row exsists update exsisting ` purchase_return_qty`` row against  `date and prodid`
   ...
   - purchase_qty - `0`
   - sale_qty - `0`
   - purchase_return_qty - `exsisting purchase_return_qty` + `qty from the purchase_return details contracts`
   - wastage_qty - `0`
   - adjust_qty - `0`
   - free_qty - `0`
   - sales_in_qty - `0`
