...
function getCartServices(fastify) {
  ...
  const getoutletmemberInfo = outletinfoServices.getOutletMembersInfoService(fastify);
   ...
   return async ({ logTrace, params = {}, query = {}, userDetails }) => {
     ...
     return {
       ...
       //cart_gr_tot: (Math.floor(netAmount * 1000) / 1000).toFixed(3),
...
function getItemsInCartlist(fastify) {
  ...
  return async ({ logTrace, params, query, userDetails }) => {
    ...
    let { users_id, counter_no, outlet_id, customer_id } = query;
    ...
    var getMemberInfo = null;
    var customer_det = null;
    if (customer_id) {
      getMemberInfo = await getoutletmemberInfo.call(knex, {
        users_id,
        counter_no,
        outlet_id,
        params: { member_id: customer_id },
      ...
      member_id = getMemberInfo.member_id;
      member_name = getMemberInfo.member_name;
      member_mobile = getMemberInfo.member_mobile;
      customer_det = getMemberInfo ? getMemberInfo : null;
    }
    console.log("customer_det", customer_det);
