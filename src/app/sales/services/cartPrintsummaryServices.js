   ...
   console.log("cccccccc", billInfo);
    ...
    if (billInfo[0].customer_id !== null) {
      const getMemberInfo = await getoutletmemberInfo.call(knex, {
        ...
        counter_no,
        outlet_id,
        params: { mobile: billInfo[0].customer_mobile_number, member_id: billInfo[0].customer_id },
        logTrace,
        userDetails
      });
    ...
    return {
      ...
      bill_details: {
        ...
        earn_point: billInfo[0].earnpoint || 0
      },
      ...
      cart_lines: billInfo.map((item, index) => ({
        ...
        scheme_discount: item.sch_discount,
...
function getcartPrintSummaryServices(fastify) {
  const { getCartPrintSummary } = billRepo(fastify);
  ...
  return async ({ body, params, logTrace, userDetails }) => {
    ...
    const response = await getCartPrintSummary.call(knex, {
      cartInfo,
      body,
      params,
      counter_no,
